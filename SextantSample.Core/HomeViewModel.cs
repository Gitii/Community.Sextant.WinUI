...
public class HomeViewModel : ViewModelBase
{
    ...
    public HomeViewModel() : base(Locator.Current.GetService<IViewStackService>())
    {
        ...
        PushGenericPage = ReactiveCommand.CreateFromObservable(
            () => ViewStackService.PushPage<GreenViewModel>(),
            outputScheduler: RxApp.MainThreadScheduler
        );

        PushPage.ThrownExceptions.Subscribe(
            error => Interactions.ErrorMessage.Handle(error).Subscribe()
        );
        PushGenericPage.ThrownExceptions.Subscribe(
            error => Interactions.ErrorMessage.Handle(error).Subscribe()
        );
        OpenModal.ThrownExceptions.Subscribe(
            error => Interactions.ErrorMessage.Handle(error).Subscribe()
        );
    }
}
